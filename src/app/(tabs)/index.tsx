...
export default function ScanScreen() {
  ...
  // Mounting CameraView isn't the same as being able to capture: the underlying
  // session (an HTMLVideoElement on web, AVCaptureSession on iOS) needs frames
  // first. Tapping before then throws "does not have enough camera data to
  // construct an image yet", so capture waits for onCameraReady.
  const [cameraReady, setCameraReady] = useState(false);

  // CameraView unmounts on blur, so readiness has to fall with it — otherwise
  // coming back to the tab leaves this true while a fresh session is still warming
  // up, which is the exact window the error comes from.
  useEffect(() => {
  ...
  const handleScan = async () => {
    if (scanning || !cameraReady || !cameraRef.current) return;
    setScanning(true);
    try {
      ...
      // onCameraReady covers the common case; this covers the rest — a session
      // interrupted mid-capture, or a feed that stalls after signalling ready.
      // Without it the rejection escapes the handler entirely and redboxes.
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.affordanceWrap, { bottom: insets.bottom + 40 }]}>
        {scanning ? (
          ...
          // Dimmed rather than hidden while warming up: the control staying put
          // means an early tap lands on a no-op instead of on whatever would have
          // reflowed into its place.
          <Pressable
            style={[styles.tapTarget, !cameraReady && styles.tapTargetWaiting]}
            onPress={handleScan}
          ...
          >
            ...
            <Text style={styles.tapLabel}>{cameraReady ? "Tap to scan" : "Starting camera…"}</Text>
          </Pressable>
        )}
      </View>
    </View>
