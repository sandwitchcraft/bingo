/**
 * ImageNet-1k class labels for `assets/models/efficientnet-lite0-int8.tflite`, in
 * class-index order. Extracted verbatim from the `labels_without_background.txt` file
 * embedded in that model's TFLite metadata — the model emits 1000 logits with **no**
 * leading background class, so index i here is output index i. (`resolveModelIO` in
 * classifier.ts still derives the offset at load rather than assuming it, so a model
 * swap to a 1001-output build keeps working.)
 *
 * A TS array rather than a bundled .txt on purpose: the frame worklet reads this on the
 * frame thread, where there is no way to await an asset read.
 *
 * Labels are the raw ImageNet strings, punctuation and capitalization included
 * ("Granny Smith"). `LABEL_TO_ITEM_KEY` keys must match them exactly.
 */
